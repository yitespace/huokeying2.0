<template>
  <div>
    ...
    <!-- 权限弹出框 -->
    <permissionForm
      v-model:visible="permissVisible"
      @refreshTable="getData"
      :data="form.val"
    />
  </div>
...
import { Lock, Edit, Search, Plus } from "@element-plus/icons-vue";

import dialogForm from "./dialog_form.vue";
import permissionForm from "./permission_form.vue";
...
// let idx: number = -1;
const handlePermiss = (index: number, row: any) => {
  getDetail(row.uid).then((res) => {
