...
<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="名称" prop="zone_name">
        <el-input
          v-model="formData.val.zone_name"
          :style="{ width: '480px' }"
          placeholder="请输入名称"
        />
      </el-form-item>
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      ...
      // if (formData.val.id) {
        rq.fetchPut(apiUrl.UPDATE_RECOMMEND, formData.val).then((res: any) => {
...
const rules = {
  ...
  zone_name: [{ required: true, message: "请输入名称", trigger: "blur" }],
